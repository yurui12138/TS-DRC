...
import sys
import logging
from sklearn.metrics import classification_report, cohen_kappa_score, confusion_matrix, accuracy_score, precision_score, recall_score, f1_score
...
def Metric(y_true, y_pred):
    ...
    # print('Accuracy: {:.1%}\tPrecision: {:.1%}\tRecall: {:.1%}\tF1: {:.1%}'.format(accuracy, macro_precision, macro_recall, weighted_f1))
...
def copy_Files(destination, data_type):
    ...
    copy("dataloader/augmentations_TS2Image.py", os.path.join(destination_dir, "augmentations_TS2Image.py"))
    copy("dataloader/dataloader.py", os.path.join(destination_dir, "dataloader.py"))
    copy(f"models/model.py", os.path.join(destination_dir, f"model.py"))
    copy("models/loss_dist.py", os.path.join(destination_dir, "loss_dist.py"))
    copy("models/ViT_model.py", os.path.join(destination_dir, "ViT_model.py"))
